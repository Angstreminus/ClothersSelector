...
func (s *Server) MustRun() {
	...
	clothRepo := repository.NewClothesRepository(dbHandler, s.Logger)
	...
	clothService := service.NewClothesService(clothRepo, s.Logger)
	...
	clothHandler := handler.NewClothesHandler(&cfg, s.Logger, clothService)
	...
	router.HandleFunc("POST /logout", userHandler.Logout)
	router.HandleFunc("POST /refresh", userHandler.Refresh)
	...
	router.HandleFunc("GET /users/{id}/presets/{preset_id}", authmiddleware.ValidateToken(presetHandler.CreatePreset))
	router.HandleFunc("DELETE /users/{id}/presets/{preset_id}", authmiddleware.ValidateToken(presetHandler.CreatePreset))
	router.HandleFunc("POST /users/{id}/presets/{preset_id}/clothes", authmiddleware.ValidateToken(clothHandler.Create))
	router.HandleFunc("GET /users/{id}/presets/{preset_id}/clothes", authmiddleware.ValidateToken(clothHandler.Create))
	router.HandleFunc("GET /users/{id}/presets/{preset_id}/clothes/{cloth_id}", authmiddleware.ValidateToken(clothHandler.Create))
